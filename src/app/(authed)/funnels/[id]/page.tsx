...
export default async function FunnelDetailPage({
  ...
  const [{ data: funnel }, { data: stages }, { data: runs }, { data: profiles }] =
    await Promise.all([
      supabase
        .from("funnels")
      ...
      supabase
        .from("funnel_runs")
        .select(
          "id, current_stage_id, outcome, pipeline_deal_id",
        )
        .eq("funnel_id", id),
  ...
  // Hydrate runs with deal info (prospect_name, company, expected_aed_monthly,
  // owner_id) — separate query because runs may have null pipeline_deal_id.
  const dealIds = (runs ?? [])
  ...
  const dealsByIdQuery =
    dealIds.length > 0
      ? await supabase
          .from("pipeline_deals")
          .select(
            "id, prospect_name, company, expected_aed_monthly, owner_id",
          )
          .in("id", dealIds)
      : { data: [] };
  ...
  const kanbanRuns: KanbanRun[] = (runs ?? []).map((r) => {
